...
///////////////////
	...
	array_init(conflict, 64);
	addr[0] = (volatile unsigned char *)lower_map;
	addr[1] = (volatile unsigned char *)lower_map;
	...
	for(set_bit = 0; set_bit < least_comp_bit ; set_bit++ ){
		...
		addr[2] = addr[0];
		addr[3] = addr[1];
		addr[2]+=(0x1LU<<set_bit);
		addr[3]+=(0x1LU<<set_bit);

		local_irq_disable();
		preempt_disable();
		conflict[set_bit]= mem_test(addr[0], addr[1], addr[2], addr[3], 1);
		preempt_enable();
		local_irq_enable();
		tmp=0;
	}
	...
	for(; set_bit <= upper_most_bit; set_bit++ ){
		addr[2] = (volatile unsigned char *)upper_map;
		addr[3] = (volatile unsigned char *)upper_map;
		addr[2]+=((0x1LU<<set_bit)-upper_addr);
		addr[3]+=((0x1LU<<set_bit)-upper_addr);
		addr[3]+=(0x1LU<<least_comp_bit);
