...
static int ioremap_core(void *unused);
...
static void write_file(char *data, int fd)
{
  //struct file *file;
  //loff_t pos = 0;
...
static int ioremap_core(void *unused)
{
	...
	printk("start!!\n");
	cpumask_clear(&cpus);
	cpumask_set_cpu(1, &cpus);

	if (sched_setaffinity(current->pid, &cpus));

	fix_addr = ioremap( (resource_size_t)0x400000000, 0x1000 );
	...
	printk("done!!\n");
	set_fs(KERNEL_DS);
	fd = sys_open("./bank_conflic_chk_128M", O_WRONLY|O_CREAT|O_TRUNC, 0644);
	for(i=0;i<idx;i++){
		sprintf(str, "%llu %u\n", i, e_time[i]);
		write_file(str, fd);
	}
...
*/
	...
	//start_addr = 0x620000000;
	//start_addr = 0x10ff00000;
